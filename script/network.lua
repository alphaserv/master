...
extinfo = {
	...
	handle_package = function(self, buff, ip, port)
	
		local package = buff:getint()
		
		if package == 0 then
			--main package
			local sub_package = buff:getint()
			if sub_package == 1 then --EXT_PLAYERSTATS
				...
				elseif version == 105 then				
					if buff:getint() == 1 then	--EXT_ERROR
						print("EXT_SERVER_ERROR could not find player with cn -1 (all players)")
						return
					end
			end
		...
		elseif package == -10 then --EXT_PLAYERSTATS_RESP_IDS
			repeat
		...
		elseif package == -11 then --EXT_PLAYERSTATS_RESP_STATS
			local table = {
				cn = buff:getint(),
				ping = buff:getint(),
			...
			print("adding player (cn="..table.cn..")")
	...
	poll_package = function(self)
		self.poller = server.interval(100, function()
			if not self then return -1 end
			if self.stop_polling then return -1 end
			
			local data, error_ = self.socket:receive()
			
			if not data then
				print("Socket error: "..error_)
			end
			
			local ip, port = self.socket:getpeername()
			
			local buff = buffer_obj(data..leftover[ip..":"..port] or "")
			
			self:handle_package(buff, ip, port)
		end)
	end,
